...
def get_changes_df():
    silver_zone = DeltaTable.forName(spark, "nyc_taxi.silver.zone")
    ...
    df_changes = spark.sql(f"""
                    SELECT 
                        ...
                        zone1,
                        zone2
                    FROM table_changes("nyc_taxi.silver.zone", {latest_version})
                """)
    return df_changes.dropDuplicates(["zone_id"])

def process_dim_zone(df):
    dim_zone = DeltaTable.forName(spark, "nyc_taxi.gold.dim_zone")
